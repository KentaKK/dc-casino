...
RegisterNetEvent('dc-casino:slots:server:spin', function(ChosenBetAmount)
    ...
    if UseCash and Player.removeMoney(SlotReferences[SlotModel].betamounts[ChosenBetAmount])
    or UseBank and Player.removeAccountMoney('bank', SlotReferences[SlotModel].betamounts[ChosenBetAmount])
    or UseItem and Player.removeInventoryItem(ItemName, SlotReferences[SlotModel].betamounts[ChosenBetAmount]) then else TriggerClientEvent('esx:showNotification', src, 'Nothing left to bet with', 'error') return end
    ...
    SetTimeout(SpinTime, function()
        ...
        if UseCash and Player.addMoney(RewardAmount)
        or UseBank and Player.addAccountMoney('bank', RewardAmount)
        or UseItem and Player.addInventoryItem(ItemName, RewardAmount) then end
    end)
end)
